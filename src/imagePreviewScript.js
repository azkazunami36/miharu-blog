...
addEventListener("load", () => {
    ...
    const images = (() => {
        ...
        for (const image of content.getElementsByTagName("img"))
            if (!image.classList.contains("ignorePreview"))
                images.push(image);
        return images;
    })();
    let viewed = false;
    class ViewElement {
        ...
        constructor() {
            ...
            this.refleshView = () => {
                ...
                if (this.nowViewed === images.length - 1)
            ...
            addEventListener("click", e => {
                ...
                if (target === null)
                    return;
                if (target.tagName === "IMG" && target.parentElement?.parentElement?.id === "content") {
                    if (viewed === false) {
                        this.nowViewed = (() => { for (let i = 0; i < images.length; i++)
